...
void Exception(const std::string& message) {
    std::ofstream file("exception.txt", std::ios::app); // 以追加模式打开文件
    if (file.is_open()) {
  ...
  if (arrayConstExpList.size() != 0) {
    ...
    if (is_global_area) {
      ...
      for (int i = 0; i < values.size(); i++) {
        ...
        if (!values[i].first)
          assert(false);
        str += std::to_string(values[i].second);
        for (auto interval : intervals) {
          if (i % interval == interval - 1)
            str += "}";
        }
...
std::vector<std::pair<bool, int>> InitValWithListAST::prepare(std::vector<int>& sizeList, int level) {
  ...
  for (auto& initVal : initValList) {
    ...
    if (typeid(*targetInitVal) == typeid(InitValAST)) {
      ...
      auto ret = ((InitValWithListAST*)initVal.get())->prepare(sizeList, targetLevel);
      vec.insert(vec.end(), ret.begin(), ret.end());
      num_cnt += ret.size();
    }
  }
  ...
  for (int i = num_cnt; i < total; i++)
    vec.push_back(std::make_pair(true, 0));
...
std::pair<bool, int> StmtWithAssignAST::Output() const {
  ...
  if (std::get<0>(fetch_result) == CONSTANT || std::get<0>(fetch_result) == UNDEFINED) {
      std::string msg = "undefined variable:" + ident + "\n";
      Exception(msg);
    assert(false);
  }
...
std::pair<bool, int> UnaryExpWithFuncAST::Output() const {
  ...
  switch (std::get<3>(result)) {
    ...
    default:
        std::string msg = "\tundefined func:" + ident + "\n";
          Exception(msg);
      assert(false);
      break;
  }
...
int search(const EqExpWithOpAST* eqExp) {
  ...
  if (eqExp->eqOp == "==")
    ...
    return lhs != rhs;
      ...
      std::string msg =  "unknown op:" + eqExp->eqOp + "\n";
      Exception(msg);
      assert(false);
  }
...
int search(const RelExpWithOpAST* relExp) {
  ...
  if (relExp->relOp == "<")
    ...
    return lhs >= rhs;
      ...
      std::string msg =  "unknown op:" + relExp->relOp + "\n";
      Exception(msg);
      assert(false);
  }
  return 0;
...
int search(const AddExpWithOpAST* addExp) {
  ...
  if (addExp->addOp == '+')
    ...
    return lhs - rhs;
      ...
      std::string msg = "unknown op:" + std::string(1, addExp->addOp) + "\n";
      Exception(msg);
      assert(false);
  }
...
int search(const MulExpWithOpAST* mulExp) {
  ...
  if (mulExp->mulOp == '*')
    ...
    return lhs % rhs;
      ...
      std::string msg = "unknown op:" + std::string(1, mulExp->mulOp) + "\n";
      Exception(msg);
      assert(false);
  }
...
int search(const UnaryExpWithOpAST* unaryExp) {
  ...
  if (unaryExp->unaryOp == '+')
    ...
    return !number;
      ...
      std::string msg = "unknown op:" + std::string(1, unaryExp->unaryOp) + "\n";
      Exception(msg);
      assert(false);
  }
...
int search(const LValAST* lVal) {
  ...
  std::string msg = "unknown value:" + lVal->ident + "\n";
  Exception(msg);
  assert(false);
  return 0;
}